...
def test_loads_all_schemas() -> None:
    ...
    discovered = sorted(path.name for path in gate.SCHEMAS_DIR.glob("*.schema.json"))
    ...
    assert "network_utility_solver_input.schema.json" in names
    assert "grid_route_solver_input.schema.json" in names
    assert "grid_route_solver_result.schema.json" in names
    assert "t6_grid_route_solver_result.schema.json" in names
    assert "hydraulic_solver_input.schema.json" in names
    assert "hydraulic_solver_result.schema.json" in names
    ...
    assert "m2_api_envelope.schema.json" in names
    assert "m2_sse_event.schema.json" in names
    assert "m2_sse_cursor.schema.json" in names
    assert "m2_page_cursor.schema.json" in names
    assert "m2_artifact_metadata.schema.json" in names
    assert "m2_control_request.schema.json" in names
    assert "m2_idempotency_record.schema.json" in names
