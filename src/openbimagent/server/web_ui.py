...
def add_web_ui(app: FastAPI, token: str | None = None) -> None:
    ...
    if (dist / "assets").is_dir():
        app.mount("/assets", StaticFiles(directory=dist / "assets"), name="frontend-dist-assets")
    token_json = json.dumps(token or "").replace("<", "\\u003c").replace(">", "\\u003e").replace("&", "\\u0026")

    @app.get("/", include_in_schema=False)
    async def _web_ui(request: Request) -> HTMLResponse:
        index = dist / "index.html"
        html = index.read_text(encoding="utf-8") if index.is_file() else PAGE
        bootstrap = f"<script>window.__WB_TOKEN = {token_json};</script>"
