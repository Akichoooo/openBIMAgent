...
from openbimagent.orchestrator.contracts import ArtifactManifest, ArtifactRecord
...
class ArtifactStore:
    ...
    def commit_file(self, agent_id: str, source: Path, *, kind: str = "output") -> ArtifactRecord:
        source = Path(source).resolve()
        if not source.is_file():
            raise FileNotFoundError(f"子代理工件不存在: {source}")
        return self.commit_bytes(agent_id, name=source.name, kind=kind, content=source.read_bytes())

    def commit_bytes(self, agent_id: str, *, name: str, kind: str, content: bytes) -> ArtifactRecord:
    ...
    def record_existing(self, path: Path, *, kind: str) -> ArtifactRecord:
