"""CompiledUtilityIR -> typed Blender execution plan v1.

This module defines the deterministic Blender host boundary used by G6. Plans contain
only allowlisted operations and data; neither the builder nor an executor may translate
them through ``execute_code`` or ``exec``.
"""

from __future__ import annotations

import hashlib
import json
import os
import re
import uuid
from enum import StrEnum
from pathlib import Path
from typing import Any, Protocol

from pydantic import BaseModel, ConfigDict, Field, model_validator

from openbimagent.utility.contracts import CompiledUtilityIR

BLENDER_PLAN_VERSION = "1.0"
BLENDER_PROTOCOL_VERSION = "1.0"
BLENDER_HOST_API_VERSION = "5.2"
BLENDER_COLLECTION = "M1-Municipal-Utility"
BLENDER_UNIT = "m"


class BlenderPlanError(ValueError):
    """Typed plan, capability, or execution validation failed closed."""


class BlenderObjectType(StrEnum):
    UTILITY_SYSTEM = "utility_system"
    MANHOLE = "manhole"
    INLET = "inlet"
    OUTLET = "outlet"
    JUNCTION = "junction"
    VALVE = "valve"
    EQUIPMENT = "equipment"
    TERMINAL = "terminal"
    DISTRIBUTION_PORT = "distribution_port"
    PIPE_SEGMENT = "pipe_segment"


class BlenderOperationKind(StrEnum):
    CREATE_OBJECT = "create_object"
    SET_PROPERTIES = "set_properties"
    CONNECT_TOPOLOGY = "connect_topology"


class BlenderReceiptStatus(StrEnum):
    COMPLETED = "completed"
    PARTIAL = "partial"
    FAILED = "failed"


class BlenderPrimitive(StrEnum):
    EMPTY = "empty"
    CYLINDER = "cylinder"
    UV_SPHERE = "uv_sphere"
    POLYLINE_CURVE = "polyline_curve"


class Coordinate(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, allow_inf_nan=False)

    x_m: float
    y_m: float
    z_m: float


class CustomProperty(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, allow_inf_nan=False)

    property_name: str = Field(
        min_length=1,
        max_length=128,
        pattern=r"^openbim_[a-z][a-z0-9_]*$",
    )
    value: str | float | int | bool


class BlenderOperation(BaseModel):
    """One allowlisted operation with a strict operation-specific shape."""

    model_config = ConfigDict(extra="forbid", frozen=True, allow_inf_nan=False)

    operation: BlenderOperationKind
    operation_id: str = Field(min_length=1, max_length=256)
    object_id: str = Field(min_length=1, max_length=256)
    object_type: BlenderObjectType
    object_name: str | None = Field(default=None, min_length=1, max_length=256)
    collection_name: str | None = Field(default=None, min_length=1, max_length=256)
    primitive: BlenderPrimitive | None = None
    units: str = Field(default=BLENDER_UNIT, pattern=r"^m$")
    position: Coordinate | None = None
    centerline: tuple[Coordinate, ...] = ()
    diameter_mm: float | None = Field(default=None, gt=0)
    material: str | None = Field(default=None, min_length=1, max_length=256)
    properties: tuple[CustomProperty, ...] = ()
    references: tuple[str, ...] = ()

    @model_validator(mode="after")
    def _validate_operation_shape(self) -> "BlenderOperation":
        if self.operation is BlenderOperationKind.CREATE_OBJECT:
            if self.object_name is None or self.collection_name is None or self.primitive is None:
                raise ValueError("create_object 缺少 object_name/collection_name/primitive")
            if self.properties or self.references:
                raise ValueError("create_object 不得携带 properties/topology 字段")
            if self.object_type is BlenderObjectType.PIPE_SEGMENT:
                if self.primitive is not BlenderPrimitive.POLYLINE_CURVE:
                    raise ValueError("pipe_segment 必须使用 polyline_curve")
                if len(self.centerline) < 2 or self.diameter_mm is None or self.material is None:
                    raise ValueError("pipe_segment 必须携带 centerline/diameter_mm/material")
                if self.position is not None:
                    raise ValueError("pipe_segment 不得同时携带 position")
            elif self.object_type is BlenderObjectType.UTILITY_SYSTEM:
                if self.primitive is not BlenderPrimitive.EMPTY:
                    raise ValueError("utility_system 必须使用 empty")
                if self.position is not None or self.centerline:
                    raise ValueError("utility_system 不应携带几何坐标")
            else:
                if self.position is None or self.centerline:
                    raise ValueError(f"{self.object_type.value} 必须只携带 position")
        elif self.operation is BlenderOperationKind.SET_PROPERTIES:
            if not self.properties:
                raise ValueError("set_properties 必须携带 properties")
            if any(value is not None for value in (self.object_name, self.collection_name, self.primitive, self.position)):
                raise ValueError("set_properties 不得携带创建字段")
            if self.centerline or self.references or self.diameter_mm is not None or self.material is not None:
                raise ValueError("set_properties 不得携带 geometry/topology 字段")
            names = [item.property_name for item in self.properties]
            if len(names) != len(set(names)):
                raise ValueError("set_properties property_name 必须唯一")
        elif self.operation is BlenderOperationKind.CONNECT_TOPOLOGY:
            if self.object_type is not BlenderObjectType.PIPE_SEGMENT:
                raise ValueError("connect_topology 只能作用于 pipe_segment")
            if len(self.references) != 2 or any(not item for item in self.references):
                raise ValueError("connect_topology 必须引用两个端口")
            if any(value is not None for value in (self.object_name, self.collection_name, self.primitive, self.position)):
                raise ValueError("connect_topology 不得携带创建字段")
            if self.centerline or self.properties or self.diameter_mm is not None or self.material is not None:
                raise ValueError("connect_topology 不得携带 geometry/properties 字段")
        return self


class BlenderExecutionPlan(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, allow_inf_nan=False)

    plan_version: str = Field(default=BLENDER_PLAN_VERSION, pattern=r"^1(?:\.\d+)?$")
    protocol_version: str = Field(default=BLENDER_PROTOCOL_VERSION, pattern=r"^1(?:\.\d+)?$")
    host_api_version: str = Field(default=BLENDER_HOST_API_VERSION, pattern=r"^5\.2$")
    plan_id: str = Field(min_length=1, max_length=256)
    ir_id: str = Field(min_length=1, max_length=256)
    source_ir_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    compiled_ir_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    units: str = Field(default=BLENDER_UNIT, pattern=r"^m$")
    collection_name: str = Field(default=BLENDER_COLLECTION, min_length=1, max_length=256)
    operations: tuple[BlenderOperation, ...] = Field(min_length=1)
    canonical_sha256: str = Field(default="", pattern=r"^(|[0-9a-f]{64})$")
    idempotency_key: str = Field(default="", pattern=r"^(|blender-plan:[0-9a-f]{64})$")

    @model_validator(mode="after")
    def _validate_plan(self) -> "BlenderExecutionPlan":
        operation_ids = [item.operation_id for item in self.operations]
        if len(operation_ids) != len(set(operation_ids)):
            raise ValueError("execution plan operation_id 必须唯一")
        if any(
            item.operation is BlenderOperationKind.CREATE_OBJECT
            and item.collection_name != self.collection_name
            for item in self.operations
        ):
            raise ValueError("create_object collection_name 必须匹配计划范围锁")
        if self.canonical_sha256:
            expected = self.compute_canonical_sha256()
            if self.canonical_sha256 != expected:
                raise ValueError("execution plan canonical_sha256 不匹配")
            if self.idempotency_key != f"blender-plan:{expected}":
                raise ValueError("execution plan idempotency_key 不匹配 canonical hash")
        return self

    def canonical_dict(self) -> dict[str, Any]:
        payload = self.model_dump(mode="json")
        payload.pop("plan_id", None)
        payload.pop("canonical_sha256", None)
        payload.pop("idempotency_key", None)
        payload["operations"] = sorted(payload["operations"], key=lambda item: item["operation_id"])
        return payload

    def canonical_json(self) -> bytes:
        return json.dumps(
            self.canonical_dict(),
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
        ).encode("utf-8")

    def compute_canonical_sha256(self) -> str:
        return hashlib.sha256(self.canonical_json()).hexdigest()

    def finalized(self) -> "BlenderExecutionPlan":
        digest = self.compute_canonical_sha256()
        return self.model_copy(
            update={
                "plan_id": f"blender-plan-{digest[:24]}",
                "canonical_sha256": digest,
                "idempotency_key": f"blender-plan:{digest}",
            }
        )


class BlenderCapabilities(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    protocol_version: str = Field(default=BLENDER_PROTOCOL_VERSION, pattern=r"^1(?:\.\d+)?$")
    host_api_version: str = Field(default=BLENDER_HOST_API_VERSION, pattern=r"^5\.2$")
    units: tuple[str, ...] = (BLENDER_UNIT, "mm")
    operations: tuple[BlenderOperationKind, ...] = tuple(BlenderOperationKind)
    object_types: tuple[BlenderObjectType, ...] = tuple(BlenderObjectType)
    primitives: tuple[BlenderPrimitive, ...] = tuple(BlenderPrimitive)
    controlled_save: bool = True
    idempotent_receipts: bool = True
    semantic_snapshot: bool = True


class BlenderOperationReceipt(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    operation_id: str
    status: BlenderReceiptStatus
    object_id: str
    host_handle: str | None = None


class BlenderExecutionReceipt(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    receipt_id: str
    plan_id: str
    idempotency_key: str
    canonical_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    status: BlenderReceiptStatus
    output_path: str
    snapshot_path: str | None = None
    state_path: str
    applied_operations: tuple[BlenderOperationReceipt, ...] = ()
    confirmed_object_ids: tuple[str, ...] = ()
    semantic_snapshot: dict[str, Any] | None = None
    errors: tuple[str, ...] = ()


class BlenderPlanExecutor(Protocol):
    def describe_capabilities(self) -> BlenderCapabilities: ...

    def execute_plan(
        self,
        plan: BlenderExecutionPlan,
        *,
        output_path: Path | str,
        approved: bool = False,
        capabilities: BlenderCapabilities | None = None,
    ) -> BlenderExecutionReceipt: ...


class BlenderBuilder:
    """Deterministically compile a validated CompiledUtilityIR into typed operations."""

    def build(
        self,
        ir: CompiledUtilityIR | dict[str, Any],
        *,
        asset_ids: list[str] | tuple[str, ...] | None = None,
    ) -> BlenderExecutionPlan:
        compiled = ir if isinstance(ir, CompiledUtilityIR) else CompiledUtilityIR.model_validate(ir)
        selected = set(asset_ids or ())
        operations: list[BlenderOperation] = []
        for system_index, system in enumerate(sorted(compiled.systems, key=lambda item: item.system_id)):
            if selected and system.system_id not in selected:
                continue
            operations.extend(_create_and_properties_system(system, f"/systems/{system_index}"))
        for node_index, node in enumerate(sorted(compiled.nodes, key=lambda item: item.node_id)):
            if selected and node.node_id not in selected:
                continue
            operations.extend(_create_and_properties_node(node, f"/nodes/{node_index}"))
            for port_index, port in enumerate(sorted(node.ports, key=lambda item: item.port_id)):
                if selected and port.port_id not in selected and node.node_id not in selected:
                    continue
                operations.extend(
                    _create_and_properties_port(
                        port,
                        node.system_id,
                        f"/nodes/{node_index}/ports/{port_index}",
                    )
                )
        for segment_index, segment in enumerate(sorted(compiled.segments, key=lambda item: item.segment_id)):
            if selected and segment.segment_id not in selected:
                continue
            operations.extend(_create_and_properties_segment(segment, f"/segments/{segment_index}"))
            operations.append(
                BlenderOperation(
                    operation=BlenderOperationKind.CONNECT_TOPOLOGY,
                    operation_id=f"connect:{segment.segment_id}",
                    object_id=segment.segment_id,
                    object_type=BlenderObjectType.PIPE_SEGMENT,
                    references=(segment.start_port_id, segment.end_port_id),
                )
            )
        if not operations:
            raise BlenderPlanError("CompiledUtilityIR 过滤后没有可编译对象")
        return BlenderExecutionPlan(
            plan_id="pending",
            ir_id=compiled.ir_id,
            source_ir_sha256=compiled.source_ir_sha256,
            compiled_ir_sha256=compiled.canonical_sha256(),
            operations=tuple(operations),
        ).finalized()

    def __call__(
        self,
        ir: CompiledUtilityIR | dict[str, Any],
        *,
        asset_ids: list[str] | tuple[str, ...] | None = None,
    ) -> BlenderExecutionPlan:
        return self.build(ir, asset_ids=asset_ids)


def validate_plan_capabilities(plan: BlenderExecutionPlan, capabilities: BlenderCapabilities) -> None:
    if plan.protocol_version != capabilities.protocol_version:
        raise BlenderPlanError("Blender execution plan protocol version 不匹配")
    if plan.host_api_version != capabilities.host_api_version:
        raise BlenderPlanError("Blender host API version 不匹配")
    if plan.units not in capabilities.units:
        raise BlenderPlanError(f"Blender 不支持计划单位: {plan.units!r}")
    if not capabilities.controlled_save or not capabilities.idempotent_receipts:
        raise BlenderPlanError("Blender capability 缺少 controlled_save/idempotent_receipts")
    allowed_ops = set(capabilities.operations)
    allowed_objects = set(capabilities.object_types)
    allowed_primitives = set(capabilities.primitives)
    known_objects = {
        op.object_id
        for op in plan.operations
        if op.operation is BlenderOperationKind.CREATE_OBJECT
    }
    for op in plan.operations:
        if op.operation not in allowed_ops:
            raise BlenderPlanError(f"Blender 能力不允许操作: {op.operation.value}")
        if op.object_type not in allowed_objects:
            raise BlenderPlanError(f"Blender 能力不允许对象类型: {op.object_type.value}")
        if op.primitive is not None and op.primitive not in allowed_primitives:
            raise BlenderPlanError(f"Blender 能力不允许 primitive: {op.primitive.value}")
        if op.operation is BlenderOperationKind.SET_PROPERTIES and op.object_id not in known_objects:
            raise BlenderPlanError(f"set_properties 引用未知对象: {op.object_id}")
        if op.operation is BlenderOperationKind.CONNECT_TOPOLOGY:
            missing = [ref for ref in op.references if ref not in known_objects]
            if missing:
                raise BlenderPlanError(f"connect_topology 引用未知端口: {missing}")


def _safe_name(value: str) -> str:
    clean = re.sub(r"[^A-Za-z0-9_-]+", "_", value).strip("_")
    if not clean:
        raise BlenderPlanError(f"对象 ID 无法形成稳定名称: {value!r}")
    return f"BL_M1_{clean}"


def _properties(
    operation_id: str,
    object_id: str,
    object_type: BlenderObjectType,
    object_kind: str,
    ifc_class: str,
    ifc_type: str | None,
    system_id: str,
    source_ir_path: str,
    domain_properties: dict[str, str | float | int | bool | None] | None = None,
    geometry_properties: dict[str, float] | None = None,
) -> BlenderOperation:
    values: dict[str, str | float | int | bool] = {
        "openbim_stable_id": object_id,
        "openbim_object_type": object_type.value,
        "openbim_object_kind": object_kind,
        "openbim_system_id": system_id,
        "openbim_ifc_class": ifc_class,
        "openbim_source_ir_path": source_ir_path,
    }
    if ifc_type is not None:
        values["openbim_ifc_predefined_type"] = ifc_type
    for name, value in sorted((domain_properties or {}).items()):
        if value is not None:
            values[f"openbim_domain_{name}"] = value
    for name, value in sorted((geometry_properties or {}).items()):
        values[f"openbim_geometry_{name}"] = value
    return BlenderOperation(
        operation=BlenderOperationKind.SET_PROPERTIES,
        operation_id=operation_id,
        object_id=object_id,
        object_type=object_type,
        properties=tuple(
            CustomProperty(property_name=name, value=value)
            for name, value in sorted(values.items())
        ),
    )


def _create_and_properties_system(system: Any, source_ir_path: str) -> list[BlenderOperation]:
    object_type = BlenderObjectType.UTILITY_SYSTEM
    return [
        BlenderOperation(
            operation=BlenderOperationKind.CREATE_OBJECT,
            operation_id=f"create:{system.system_id}",
            object_id=system.system_id,
            object_type=object_type,
            object_name=_safe_name(system.system_id),
            collection_name=BLENDER_COLLECTION,
            primitive=BlenderPrimitive.EMPTY,
        ),
        _properties(
            f"properties:{system.system_id}",
            system.system_id,
            object_type,
            "system",
            system.ifc_class,
            system.ifc_predefined_type,
            system.system_id,
            source_ir_path,
            {"flow_regime": system.flow_regime.value, "system_type": system.system_type.value},
        ),
    ]


def _create_and_properties_node(node: Any, source_ir_path: str) -> list[BlenderOperation]:
    object_type = BlenderObjectType(node.node_type.value)
    return [
        BlenderOperation(
            operation=BlenderOperationKind.CREATE_OBJECT,
            operation_id=f"create:{node.node_id}",
            object_id=node.node_id,
            object_type=object_type,
            object_name=_safe_name(node.node_id),
            collection_name=BLENDER_COLLECTION,
            primitive=BlenderPrimitive.CYLINDER,
            position=Coordinate(**node.position.model_dump()),
        ),
        _properties(
            f"properties:{node.node_id}",
            node.node_id,
            object_type,
            "node",
            node.ifc_class,
            node.ifc_predefined_type,
            node.system_id,
            source_ir_path,
            {"ground_elevation_m": node.ground_elevation_m, "node_type": node.node_type.value},
        ),
    ]


def _create_and_properties_port(port: Any, system_id: str, source_ir_path: str) -> list[BlenderOperation]:
    object_type = BlenderObjectType.DISTRIBUTION_PORT
    return [
        BlenderOperation(
            operation=BlenderOperationKind.CREATE_OBJECT,
            operation_id=f"create:{port.port_id}",
            object_id=port.port_id,
            object_type=object_type,
            object_name=_safe_name(port.port_id),
            collection_name=BLENDER_COLLECTION,
            primitive=BlenderPrimitive.UV_SPHERE,
            position=Coordinate(**port.position.model_dump()),
        ),
        _properties(
            f"properties:{port.port_id}",
            port.port_id,
            object_type,
            "port",
            port.ifc_class,
            None,
            system_id,
            source_ir_path,
            {"direction": port.direction.value},
        ),
    ]


def _create_and_properties_segment(segment: Any, source_ir_path: str) -> list[BlenderOperation]:
    object_type = BlenderObjectType.PIPE_SEGMENT
    return [
        BlenderOperation(
            operation=BlenderOperationKind.CREATE_OBJECT,
            operation_id=f"create:{segment.segment_id}",
            object_id=segment.segment_id,
            object_type=object_type,
            object_name=_safe_name(segment.segment_id),
            collection_name=BLENDER_COLLECTION,
            primitive=BlenderPrimitive.POLYLINE_CURVE,
            centerline=tuple(Coordinate(**point.model_dump()) for point in segment.centerline),
            diameter_mm=segment.diameter_mm,
            material=segment.material,
        ),
        _properties(
            f"properties:{segment.segment_id}",
            segment.segment_id,
            object_type,
            "segment",
            segment.ifc_class,
            segment.ifc_predefined_type,
            segment.system_id,
            source_ir_path,
            {"min_cover_depth_m": segment.min_cover_depth_m},
            {
                "diameter_mm": segment.diameter_mm,
                "end_invert_m": segment.end_invert_m,
                "horizontal_length_m": segment.horizontal_length_m,
                "slope": segment.slope,
                "start_invert_m": segment.start_invert_m,
            },
        ),
    ]


class FakeBlenderExecutor:
    """Offline typed executor with durable state for idempotency and restart tests."""

    def __init__(self, *, fail_after_operations: int | None = None, state_path: Path | None = None) -> None:
        self.capabilities = BlenderCapabilities()
        self.fail_after_operations = fail_after_operations
        self.state_path = Path(state_path).resolve() if state_path is not None else None
        self.objects: dict[str, BlenderOperation] = {}
        self.properties: dict[str, tuple[CustomProperty, ...]] = {}
        self.connections: dict[str, tuple[str, ...]] = {}
        self._applied: dict[str, set[str]] = {}
        self._plan_hashes: dict[str, str] = {}
        self._receipts: dict[str, BlenderExecutionReceipt] = {}
        self.apply_calls = 0
        if self.state_path is not None and self.state_path.is_file():
            self._load_state()

    def describe_capabilities(self) -> BlenderCapabilities:
        return self.capabilities

    def execute_plan(
        self,
        plan: BlenderExecutionPlan,
        *,
        output_path: Path | str = "offline.blend",
        approved: bool = False,
        capabilities: BlenderCapabilities | None = None,
    ) -> BlenderExecutionReceipt:
        del approved
        plan = BlenderExecutionPlan.model_validate(plan.model_dump(mode="json"))
        validate_plan_capabilities(plan, capabilities or self.capabilities)
        known_hash = self._plan_hashes.get(plan.idempotency_key)
        if known_hash is not None and known_hash != plan.canonical_sha256:
            raise BlenderPlanError("同一幂等键对应不同 execution plan，拒绝执行")
        self._plan_hashes[plan.idempotency_key] = plan.canonical_sha256
        previous = self._receipts.get(plan.idempotency_key)
        if previous is not None:
            return previous
        applied = self._applied.setdefault(plan.idempotency_key, set())
        operation_receipts: list[BlenderOperationReceipt] = []
        errors: list[str] = []
        for operation in plan.operations:
            if operation.operation_id in applied:
                operation_receipts.append(self._operation_receipt(operation))
                continue
            if self.fail_after_operations is not None and len(applied) >= self.fail_after_operations:
                errors.append(f"注入中断: operation={operation.operation_id}")
                break
            self._apply(operation)
            applied.add(operation.operation_id)
            self._persist_state()
            operation_receipts.append(self._operation_receipt(operation))
        complete = len(applied) == len(plan.operations)
        receipt = BlenderExecutionReceipt(
            receipt_id=f"blender-receipt-{plan.canonical_sha256[:24]}",
            plan_id=plan.plan_id,
            idempotency_key=plan.idempotency_key,
            canonical_sha256=plan.canonical_sha256,
            status=BlenderReceiptStatus.COMPLETED if complete else BlenderReceiptStatus.PARTIAL,
            output_path=str(Path(output_path)),
            state_path=str(self.state_path or Path(str(output_path) + ".openbimagent.json")),
            applied_operations=tuple(operation_receipts),
            confirmed_object_ids=tuple(sorted(self.objects)),
            errors=tuple(errors),
        )
        if complete:
            self._receipts[plan.idempotency_key] = receipt
        self._persist_state()
        return receipt

    def _operation_receipt(self, operation: BlenderOperation) -> BlenderOperationReceipt:
        created = self.objects.get(operation.object_id)
        return BlenderOperationReceipt(
            operation_id=operation.operation_id,
            status=BlenderReceiptStatus.COMPLETED,
            object_id=operation.object_id,
            host_handle=created.object_name if created else None,
        )

    def _apply(self, operation: BlenderOperation) -> None:
        self.apply_calls += 1
        if operation.operation is BlenderOperationKind.CREATE_OBJECT:
            existing = self.objects.get(operation.object_id)
            if existing is not None and existing != operation:
                raise BlenderPlanError(f"对象 {operation.object_id!r} 已存在但语义不同")
            self.objects[operation.object_id] = operation
        elif operation.operation is BlenderOperationKind.SET_PROPERTIES:
            if operation.object_id not in self.objects:
                raise BlenderPlanError(f"set_properties 对象不存在: {operation.object_id}")
            self.properties[operation.object_id] = operation.properties
        elif operation.operation is BlenderOperationKind.CONNECT_TOPOLOGY:
            if operation.object_id not in self.objects or any(ref not in self.objects for ref in operation.references):
                raise BlenderPlanError(f"connect_topology 引用对象不存在: {operation.object_id}")
            self.connections[operation.object_id] = operation.references

    def _load_state(self) -> None:
        assert self.state_path is not None
        try:
            payload = json.loads(self.state_path.read_text(encoding="utf-8"))
            if payload.get("state_version") != "1.0":
                raise ValueError("unsupported state_version")
            self.objects = {
                key: BlenderOperation.model_validate(value)
                for key, value in payload.get("objects", {}).items()
            }
            self.properties = {
                key: tuple(CustomProperty.model_validate(value) for value in values)
                for key, values in payload.get("properties", {}).items()
            }
            self.connections = {
                key: tuple(str(value) for value in values)
                for key, values in payload.get("connections", {}).items()
            }
            self._applied = {
                key: {str(value) for value in values}
                for key, values in payload.get("applied", {}).items()
            }
            self._plan_hashes = {
                str(key): str(value)
                for key, value in payload.get("plan_hashes", {}).items()
            }
            self._receipts = {
                key: BlenderExecutionReceipt.model_validate(value)
                for key, value in payload.get("receipts", {}).items()
            }
        except Exception as exc:
            raise BlenderPlanError(f"Blender fake host 状态损坏: {self.state_path}: {exc}") from exc

    def _persist_state(self) -> None:
        if self.state_path is None:
            return
        self.state_path.parent.mkdir(parents=True, exist_ok=True)
        payload = {
            "state_version": "1.0",
            "objects": {
                key: value.model_dump(mode="json")
                for key, value in sorted(self.objects.items())
            },
            "properties": {
                key: [value.model_dump(mode="json") for value in values]
                for key, values in sorted(self.properties.items())
            },
            "connections": {
                key: list(values)
                for key, values in sorted(self.connections.items())
            },
            "applied": {
                key: sorted(values)
                for key, values in sorted(self._applied.items())
            },
            "plan_hashes": dict(sorted(self._plan_hashes.items())),
            "receipts": {
                key: value.model_dump(mode="json")
                for key, value in sorted(self._receipts.items())
            },
        }
        encoded = (
            json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True, allow_nan=False)
            + "\n"
        ).encode("utf-8")
        temporary = self.state_path.with_name(f".{self.state_path.name}.{uuid.uuid4().hex}.tmp")
        try:
            with temporary.open("xb") as file:
                file.write(encoded)
                file.flush()
                os.fsync(file.fileno())
            os.replace(temporary, self.state_path)
        finally:
            if temporary.exists():
                temporary.unlink()


__all__ = [
    "BLENDER_COLLECTION",
    "BLENDER_HOST_API_VERSION",
    "BLENDER_PLAN_VERSION",
    "BLENDER_PROTOCOL_VERSION",
    "BlenderBuilder",
    "BlenderCapabilities",
    "BlenderExecutionPlan",
    "BlenderExecutionReceipt",
    "BlenderObjectType",
    "BlenderOperation",
    "BlenderOperationKind",
    "BlenderOperationReceipt",
    "BlenderPlanError",
    "BlenderPrimitive",
    "BlenderReceiptStatus",
    "Coordinate",
    "CustomProperty",
    "FakeBlenderExecutor",
    "validate_plan_capabilities",
]
