...
版本:v0.8.2(municipal clash-free v0)· 2026-08-01 · 姊妹篇:[ARCHITECTURE.md](ARCHITECTURE.md)
...
- `solver.py` 定义 `StraightGravitySolverInput v0.2` 与 `municipal-straight-gravity-solver v0.2.0`：限定两井一直管 DN300 混凝土污水，根据两端平面坐标、地面标高、设计坡度和地表类型计算最浅合规内底标高；若给定起点内底则保留输入并以 Evidence 报告结果。
- v0.2 增加 `collision_context`、`AxisAlignedBoxObstacle`、`ExistingPipeObstacle` 和显式 `ClearanceRule`。上下文缺失时 `clash_free=UNKNOWN`；`coverage=complete` 时对设计管道圆柱包络与 AABB/既有直圆管胶囊体计算三维实体表面最短净距。每个障碍物生成一条 RuleEvidence，任一 FAIL 即聚合失败；显式完整空清单可判定 PASS。净距等于限值按 `CLASH_TOLERANCE_M=1e-6` 通过。
- 障碍物保持为 Solver Context，不进入 `CompiledUtilityIR` 的交付实体集合；Evidence 保存障碍物 ID、实测净距、限值和调用方已选择的规则条款。Solver 不从类别或自然语言推断适用净距。
- Solver v0.2 生成管径、坡度、覆土、井距和碰撞 PASS/FAIL Evidence；缺少水力参数时水力 Evidence 仍为 UNKNOWN。默认生产 Domain Gate 要求 `clash_free`，因此碰撞上下文缺失不会被误报为可交付。
