...
- **审批中心已打通（P0 完成，撤掉 yes=True）**：`server/approvals.py` 阻塞式 Web 审批门——pipeline 触门（execute_code/deliver 前）挂起运行线程，`GET /api/v1/approvals` 轮询票据，前端 HITL 卡人工批准/拒绝 → `POST …/decide` 放行；approval_requested/decided 事件落 Session JSONL（对齐 decision_receipt）；30min 超时失败关闭。`tests/test_approvals.py`（3 测 E2E：真跑 single_asset_hero → deliver 门挂起 → 批准 → 放行 → 事件落盘）。已知边界：municipal_utility 离线运行在 domain_gate 停（UNKNOWN，solver 证据键缺，CLI 主线需 utility_solver_input——Web 运行待补该入参）。旧原型 A–H 已清理，ui/ 保留方案 I/J/K/L 与装配脚本 `scratch/build_web_ui.py`。
