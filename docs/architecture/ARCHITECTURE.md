...
- P1b-A 将 background 可变状态原子写入 `sessions/_runtime/<request_id>.json`。运行中可用稳定幂等键把宿主 receipt/state 提交为不可变 `partial` checkpoint，RuntimeState 只保存受 Schema 约束的工件记录。启动时：终态直接恢复且不重跑模型；`finalizing` 幂等补齐 lifecycle/receipt；遗留 `prepared/running` 以 `RuntimeRestarted(retryable=true)` 失败关闭，并把 checkpoint 纳入 failed recovery Manifest，避免丢失已发生的副作用事实；损坏状态文件严格失败并报告路径。
